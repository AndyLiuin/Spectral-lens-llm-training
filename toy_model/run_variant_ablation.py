...
def main() -> None:
    parser = argparse.ArgumentParser(description="Variant ablation for toy Transformer track.")
    ...
    parser.add_argument("--variants", type=str, default="baseline,rope,muon,untie_embed,value_mix,unet,fixed_window,attn_scale")
    ...
    for track in tracks:
        for variant in variants:
            ...
            for seed in seeds:
                ...
                state = train_toy_run(
                    config=cfg,
                    ablation_name="variant_ablation",
                    write_summary=True,
                    save_spectra=True,
                    device=args.device,
                )
    ...
    for track in tracks:
        for variant in variants:
            if variant == "baseline":
                continue
            # Pair baseline-vs-variant by seed.
            baseline_dfs = []
            variant_dfs = []
            for seed in seeds:
                kb = (track, "baseline", seed)
                kv = (track, variant, seed)
    ...
    agg_matched = (
        matched_df.groupby(["track", "variant", "matched_loss_target"], as_index=False)
        .agg(
        ...
        else pd.DataFrame(columns=["track", "variant", "matched_loss_target", "js_div_mean", "js_div_std", "n"])
    )

    out_dir = Path(args.out_root) / "variant_ablation"
    out_dir.mkdir(parents=True, exist_ok=True)
    summary_df.to_csv(out_dir / "variant_ablation_summary.csv", index=False)
    matched_df.to_csv(out_dir / "variant_ablation_matched_pairs.csv", index=False)
    agg_matched.to_csv(out_dir / "variant_ablation_matched_aggregate.csv", index=False)

    print(f"Wrote: {out_dir / 'variant_ablation_summary.csv'}")
    print(f"Wrote: {out_dir / 'variant_ablation_matched_pairs.csv'}")
    print(f"Wrote: {out_dir / 'variant_ablation_matched_aggregate.csv'}")
