...
"""
GPT training with AdamW only (streaming from HuggingFace FineWeb)

This is a modified version of your Muon script with:
- Muon removed entirely
- a single AdamW optimizer over all parameters
- weight decay applied to matrix parameters only
- model/init matched more closely to your AdamW baseline:
    * attention output proj uses bias=True
    * no zero-init residual projections
    * custom GPT-style initialization with residual scaling
"""
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # Matched more closely to your AdamW baseline
...
def get_args():
    import argparse
    p = argparse.ArgumentParser("GPT-2 Training with AdamW (Streaming)")
...
def save_training_checkpoint(step, raw_model, optimizer, out_dir, val_loss=None, extra=None, filename=None):
...
def main():
    ...
    print0(f"\nOptimization:")
    print0(f"  AdamW LR: {args.learning_rate}")
    ...
    while True:
        ...
        if run_validation:
            ...
            if args.stop_mode == "const_loss":
                if (args.loss_threshold > 0) and (val_loss < args.loss_threshold) and (step > 0):
                    ...
                    if master_process:
                        ckpt_path = save_training_checkpoint(step, raw_model, optimizer, args.output_dir, val_loss)
        ...
        if master_process and checkpoint_every > 0 and (step == 0 or (step + 1) % checkpoint_every == 0):
            ckpt_path = save_training_checkpoint(step + 1 if step > 0 else 0, raw_model, optimizer, args.output_dir, val_loss)
            print0(f"   Checkpoint saved: {ckpt_path}")
