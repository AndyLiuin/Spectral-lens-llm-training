...
import tiktoken # added for sampling
...
# Define rms_norm as used in the script
def rms_norm(x, norm_shape, eps=1e-6):
...
class GPT(nn.Module):
    ...
    def forward(self, idx, target):
        ...
        # Decoder pass - process the remaining blocks with weighted skip 
...
# Make sure this is set BEFORE wandb.init so code saving / notebook name detection behaves
os.environ["WANDB_NOTEBOOK_NAME"] = "8_unet"

import argparse

def get_args():
    parser = argparse.ArgumentParser(description="8_unet GPT-2 training")
    ...
    parser.add_argument("--total_batch_size", type=int, default=524288,
                        help="Total batch size in tokens (512 * 1024)")
    parser.add_argument("--max_device_batch_size", type=int, default=64)

    # optimization
    parser.add_argument("--embed_lr", type=float, default=0.6,
                        help="Adam LR for wte embeddings")
    parser.add_argument("--head_lr", type=float, default=0.008,
                        help="Adam LR for lm_head")
    parser.add_argument("--muon_lr", type=float, default=0.04,
                        help="Muon LR for matrix params")
    parser.add_argument("--scalar_lr", type=float, default=0.04,
                        help="Adam LR for scalar params")

    # Muon momentum warmup
    parser.add_argument("--muon_momentum_init", type=float, default=0.85)
    parser.add_argument("--muon_momentum_final", type=float, default=0.95)
    parser.add_argument("--muon_momentum_warmup_steps", type=int, default=500)

    # LR schedule (as fractions)
    parser.add_argument("--warmup_frac", type=float, default=0.0,
                        help="Fraction of total steps for warmup")
    parser.add_argument("--warmdown_frac", type=float, default=0.1)
    ...
    parser.add_argument("--tensorcores", action="store_true",
                        help="Set float32 matmul precision to high")
    parser.add_argument("--use_cudnn_attn", action="store_true",
                        help="Use CUDNN attention backend")

    # wandb
    parser.add_argument("--wandb_project", type=str, default="gpt2-dynamics")
    parser.add_argument("--wandb_entity", type=str, default=None)
    parser.add_argument("--wandb_run_name", type=str, default="8_unet_gpt2")
    parser.add_argument("--wandb_tags", type=str, nargs="*",
                        default=["fineweb", "gpt2", "unet", "muon"])
...
def main():
    ...
    print0(f"\n{'='*60}")
    print0("CONFIGURATION: UNET (8_unet) - DOUBLE LR")
    print0(f"{'='*60}")
    print0(f"Device: {device} | DDP: {ddp} | World Size: {ddp_world_size}")
    print0(f"\nBatching:")

    B, T = args.batch_size, args.sequence_length
    micro_B = min(args.max_device_batch_size, B)
    while B % micro_B != 0 and micro_B > 1:
        micro_B //= 2
    num_micro = max(1, B // micro_B)

    print0(f"  Device batch size: {B} sequences")
    print0(f"  Micro batch size: {micro_B} sequences ({num_micro} micro-batches)")
    ...
    print0(f"\nOptimization (4 Optimizers - UNet Double-LR):")
    ...
    # You must have these defined elsewhere in your notebook/script:
    # - GPTConfig
    # - GPT
    ...
    print0(f"  Architecture: UNet (skip connections with learnable weights)")
    ...
    # IMPORTANT: compile AFTER setting SDPA backend flags (helps avoid kernel-selection weirdness)
    if args.compile:
    ...
    val_loader = DistributedDataLoader(args.input_val_bin, B, T, ddp_rank, ddp_world_size) if args.input_val_bin else None
    ...
    print0(f"\nConfiguring 4-optimizer setup (UNet double-LR)...")
    ...
    if master_process:
        ...
        # keep wandb files in output_dir on cluster
    ...
    while True:
        ...
        if run_validation:
            ...
            if master_process and wandb_run is not None:
                wandb.log({
                    ...
                    # canonical + namespaced loss keys
                    "loss/val": val_loss,
                    "val/loss": val_loss,
        ...
        for gas in range(grad_accum_steps):
            xb_splits = x.split(micro_B, dim=0)
            yb_splits = y.split(micro_B, dim=0)

            for i, (xb, yb) in enumerate(zip(xb_splits, yb_splits)):
                last_backward = (gas == grad_accum_steps - 1) and (i == num_micro - 1)
                if ddp:
                    model.require_backward_grad_sync = last_backward
                with ctx:
                    loss = model(xb, yb)
                    (loss / (grad_accum_steps * num_micro)).backward()
