...
def get_args():
    ...
    p.add_argument("--learning_rate", type=float, default=6e-4,
                   help="Shared base LR for AdamW and Muon.")
    p.add_argument("--muon_lr_scale", type=float, default=0.2,
                   help="Muon h scale in adjusted_lr = learning_rate * h * sqrt(max(m, n)).")
...
def main():
    ...
    print0(f"\nOptimization:")
    print0(f"  Shared base LR: {args.learning_rate}")
    print0(f"  Muon LR scale (h): {args.muon_lr_scale}")
    ...
    # Paper-style Muon scaling:
    # adjusted_lr = base_lr * h * sqrt(max(m, n)) for each 2D hidden-layer matrix.
    muon_param_groups = []
    muon_adjusted_lrs = []
    for p in raw_model.transformer.h.parameters():
        ...
        if p.ndim != 2:
            continue
        m, n = p.shape
        adjusted_lr = args.learning_rate * args.muon_lr_scale * math.sqrt(max(m, n))
        muon_param_groups.append({"params": [p], "lr": adjusted_lr})
        muon_adjusted_lrs.append(adjusted_lr)
    ...
    if len(muon_adjusted_lrs) > 0:
        print0(f"  Muon adjusted LR range: [{min(muon_adjusted_lrs):.6g}, {max(muon_adjusted_lrs):.6g}]")
        print0(f"  Muon matrices: {len(muon_adjusted_lrs)}")
