...
def as_col_major(t: Tensor) -> Tensor:
    # Produce a column-major (Fortran-order) view with the same shape
    # (stride becomes (1, rows) for 2D), satisfying _scaled_mm's mat2 requirement.
...
def mm_op(x: Tensor, w: Tensor, x_s: float, w_s: float, grad_s: float) -> tuple[Tensor, Tensor, Tensor]:
    """
    Forward FP8 matmul:
      inputs:  x  (N, d_in)  bfloat16
               w  (d_out, d_in) bfloat16
      returns: (out_bf16, x_f8, w_f8)
    """
    @torch.compile
    def impl(_x: Tensor, _w: Tensor):
        # _scaled_mm expects: self=row-major, mat2=col-major
        assert _x.is_contiguous() and _w.is_contiguous()
        x_f8 = _x.mul(x_s).to(torch.float8_e4m3fn)      # (N, d_in), row-major
        w_f8 = _w.mul(w_s).to(torch.float8_e4m3fn)      # (d_out, d_in), row-major
        out = torch._scaled_mm(
            x_f8,                 # row-major (N, d_in)
            w_f8.t(),            # col-major (d_in, d_out) via transpose
...
# Fake (meta) kernel for compile/tracing (required by torch.compile)
@mm_op.register_fake
def _(x: Tensor, w: Tensor, *_):
    # Validate basic invariants (FakeTensor-friendly checks)
    torch._check(x.ndim == 2 and w.ndim == 2)
    torch._check(x.shape[1] == w.shape[1])
    torch._check(x.device == w.device)
    # Return tensors with correct *metadata* (shapes/dtypes/devices); no real data needed.
...
# -----------------------------------------------------------------------------
# Backward FP8 op (uses col-major fix for mat2)

@torch.library.custom_op("nanogpt::mm_backward", mutates_args=())
def mm_backward_op(g: Tensor, x_f8: Tensor, w_f8: Tensor,
                   x_s: float, w_s: float, grad_s: float) -> tuple[Tensor, Tensor]:
    """
    Backward for FP8 matmul:
      g:    (N, d_out)  bfloat16/bfloat16-like
      x_f8: (N, d_in)   float8_e4m3fn
      w_f8: (d_out, d_in) float8_e4m3fn
      returns: (grad_x_bf16, grad_w_fp32)
    """
    @torch.compile
    def impl(grad: Tensor, _x_f8: Tensor, _w_f8: Tensor):
        ...
        grad_x = torch._scaled_mm(
            grad_f8.contiguous(),   # row-major
            as_col_major(_w_f8),    # col-major (critical for _scaled_mm)
            out_dtype=torch.bfloat16,
            scale_a=grad_inv_s,
            scale_b=w_inv_s,
        ...
        grad_w = torch._scaled_mm(
            grad_f8.t().contiguous(),  # row-major
            as_col_major(_x_f8),       # col-major
...
# Fake kernel for the backward op (compile-time tracing)
@mm_backward_op.register_fake
def _(g: Tensor, x_f8: Tensor, w_f8: Tensor, *_):
    # Return shapes/dtypes that match real backward: (grad_x bf16, grad_w fp32)
    grad_x_meta = x_f8.to(torch.bfloat16)
    grad_w_meta = w_f8.to(torch.float32)
    return grad_x_meta, grad_w_meta

# -----------------------------------------------------------------------------
# Autograd + FP8 LM head wrapper
...
# Attach autograd to the forward op
...
class Muon(torch.optim.Optimizer):
    ...
    Supports per-parameter lr_mul and wd_mul attributes for sub learning rate multiplication.
    """
    def __init__(
        self,
        params,
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Per-parameter sub learning rate and weight decay multipliers
                eff_lr = lr * max(1.0, p.size(-2) / p.size(-1)) ** 0.5 * getattr(p, "lr_mul", 1.0)
                eff_wd = lr * weight_decay * getattr(p, "wd_mul", 1.0)
                ...
                # Note: removed the max(1, size) scale from the update since it's now in eff_lr

                p.add_(gg.to(dtype=p.dtype), alpha=-eff_lr)


class AdamWithSubLR(torch.optim.Optimizer):
    """
    Adam optimizer with support for per-parameter lr_mul and wd_mul attributes.
    
    This allows using a single optimizer with different effective learning rates
    for different parameters, which can be more efficient than using multiple
    separate optimizers.
    """
    def __init__(
        self,
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                step_size = eff_lr * (torch.sqrt(bias2) / bias1)
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, max_seq_len: int):
        ...
        self.mix.lr_mul = 5.0  # sub learning rate multiplier
...
class Block(nn.Module):
    def __init__(self, dim: int, n_head: int, layer_idx: int, max_seq_len: int, skip_attn_layer: int = 7):
        ...
        self.lambdas.lr_mul = 5.0  # sub learning rate multiplier
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig, lm_head_softcap: float = 30.0, eot_token: int = 50256):
        ...
        self.skip_weights.lr_mul = 5.0  # sub learning rate multiplier

        self.transformer = nn.ModuleDict(dict(
            wte=nn.Embedding(config.vocab_size, config.n_embd),
            h=nn.ModuleList([
                Block(config.n_embd, config.n_head, i, max_seq_len=config.seq_len, skip_attn_layer=7)
                for i in range(config.n_layer)
            ]),
        ))
        # Set lr_mul for embedding parameters
        ...
        # Set lr_mul for value embedding parameters
        for param in self.value_embeds.parameters():
            param.lr_mul = 75.0

        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
        self.lm_head.weight.data.zero_()
        self.lm_head.weight.lr_mul = 27.5  # sub learning rate multiplier

    def _make_long_short_block_masks(self, tokens_1d: Tensor, sliding_window_num_blocks: Tensor) -> tuple[BlockMask, BlockMask]:
        """
        Long-short window attention masks (Gemma-2 style):
          - long_bm : uses sliding_window_num_blocks
          - short_bm: uses sliding_window_num_blocks // 2

        Keeps script-1 naming conventions and EOT-based doc boundaries,
        while matching the clamp-based construction idea from script-2.
        """
        ...
        # token-level causal + same-doc constraint (used by BlockMask.mask_mod)
        ...
        # document overlap at block level (same as your script 1)
        ...
        # "base" masks without any windowing; windowing will be enforced via kv_num_blocks clamping (script-2 style)
        nonzero_bm = causal_bm & document_bm
        full_bm = causal_full_bm & document_full_bm

        def dense_to_ordered(dense_mask: Tensor):
            num_blocks = dense_mask.sum(dim=-1, dtype=torch.int32)
            # Put "True" entries first AND in descending KV index order (most recent first)
        ...
        def build_bm(sw_num_blocks: Tensor) -> BlockMask:
            # clamp logic mirrors script-2 (windowing via kv block counts, keeping "full" separate)
        ...
        long_bm = build_bm(sw_long)
        short_bm = build_bm(sw_short)
        return long_bm, short_bm

    def _layer_blockmask_schedule(self, long_bm: BlockMask, short_bm: BlockMask, n: int) -> list[BlockMask]:
        """
        Generalizes your script-2 schedule:
            [long, short, short, short, long, short]  (for n=6)

        For other n, we repeat the base motif [long, short, short, short]
    ...
    def forward(self, inputs: Tensor, targets: Tensor, sliding_window_num_blocks: Tensor) -> Tensor:
        ...
        # long-short window masks
        long_bm, short_bm = self._make_long_short_block_masks(inputs, sliding_window_num_blocks)
        ...
        # decoder: reverse schedule (script-2 behavior)
        dec_masks = list(reversed(enc_masks))
        # If decoder has a different length than encoder (odd n_layer), extend its schedule independently
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = self.transformer.h[j](x, ve_all[j], x0, dec_masks[i])
        ...
        if sc > 0:
            logits = (2 * sc) * torch.sigmoid(logits / (0.5 * sc * x.size(-1)**0.5))
...
def get_args():
    ...
    p.add_argument("--lm_head_softcap", type=float, default=15)
    ...
    # LRs
    p.add_argument("--embed_lr", type=float, default=0.6, help="Learning rate for embeddings")
    p.add_argument("--head_lr", type=float, default=0.008, help="Learning rate for LM head")
    p.add_argument("--muon_lr", type=float, default=0.04, help="Learning rate for Muon optimizer")
    p.add_argument("--scalar_lr", type=float, default=0.04, help="Learning rate for scalar parameters")
    # Legacy adam_lr argument for backwards compatibility
    p.add_argument("--adam_lr", type=float, default=None, help="[Deprecated] Base learning rate for Adam")
    ...
    p.add_argument("--window_min", type=int, default=128)
    p.add_argument("--window_max", type=int, default=1792)
    p.add_argument("--window_warmup_steps", type=int, default=2500)
    ...
    p.add_argument("--wandb_run_name", type=str, default="19_sublr_gpt2")
...
def main():
    ...
    # ---------------------------
    # Optimizers (4-way like 17_lswa.py)
    # ---------------------------
    # Separate parameter groups with individual learning rates
    ...
    # Create 4 optimizers with individual learning rates
    ...
    if master_process and args.wandb_mode != "disabled":
        ...
        wandb_run = wandb.init(
            ...
            tags=["vte", "flex_attention", "unet", "trunc_rope", "ref_window_schedule"],
    ...
    while True:
        ...
        # Calculate epoch progress for warmup and stopping
        ...
        sw_blocks = int(math.ceil(window_tokens_raw / args.block_size))
        sw_blocks = max(1, min(sw_blocks, 14))

        window_tokens = sw_blocks * args.block_size
        ...
        if run_val:
            ...
            log_print(f"      window {window_tokens} tok = {sw_blocks} blocks (ref-like: 128→1792 over warmup)")
        ...
        # Update Muon momentum for ALL param groups (safe even if only one group)
