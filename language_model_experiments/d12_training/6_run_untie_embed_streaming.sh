...
#SBATCH --job-name=gpt2_6_untie_streaming
#SBATCH --output=logs/6_untie_streaming_%A.out
#SBATCH --error=logs/6_untie_streaming_%A.err
...
# Clear any corrupted PyTorch Inductor cache from previous runs
INDUCTOR_CACHE="${TMPDIR}/torchinductor_${USER}"
if [ -d "${INDUCTOR_CACHE}" ]; then
  echo "Clearing previous Inductor cache: ${INDUCTOR_CACHE}"
  rm -rf "${INDUCTOR_CACHE}"
fi

# Disable remote cache to prevent NFS corruption issues with torch.compile
...
# Configuration
# ----------------------------
PY_SCRIPT="6_untie_embed_streaming.py"
...
# Output directory
OUT_DIR="/nfs/roberts/scratch/pi_jks79/zl664/Scaling_final/const_loss_run_gpt2s/6_untie_streaming_gpt2/bs524288_embLR0.3_headLR0.002_muonLR0.02"

# Model architecture (GPT-2 Small with untied embeddings)
VOCAB_SIZE=50304
N_LAYER=12
N_HEAD=6
N_EMBD=768
...
# 3-Optimizer Learning Rates (Untied Embeddings)
EMBED_LR=0.3
HEAD_LR=0.002
MUON_LR=0.02
MUON_MOMENTUM=0.95
...
# ----------------------------
echo "==========================================="
echo "Starting 1-GPU Training: Untied Embeddings + Muon (Streaming)"
echo "==========================================="
...
echo ""
echo "Architecture: GPT-2 Small, UNTIED embeddings"
...
echo ""
echo "Optimizer (3-group):"
echo "  1. Adam for wte (embed_lr=${EMBED_LR})"
echo "  2. Adam for lm_head (head_lr=${HEAD_LR})"
echo "  3. Muon for hidden (muon_lr=${MUON_LR}, momentum=${MUON_MOMENTUM})"
echo "==========================================="
echo ""
