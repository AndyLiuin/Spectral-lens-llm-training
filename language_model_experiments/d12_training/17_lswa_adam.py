...
# Tip: avoid double-compiling flex_attention AND the whole model.
# We'll conditionally compile flex_attention later based on args.compile.
...
def get_args():
    ...
    p.add_argument("--matrix_lr", type=float, default=0.004)
    ...
    p.add_argument("--wandb_run_name", type=str, default="17_lswa_adam_gpt2")
...
def main():
    ...
    opt_matrix = torch.optim.Adam(
        matrix_params,
        lr=args.matrix_lr, betas=(0.9, 0.95),
        fused=(device_type == "cuda")
    )
    ...
    if master_process and args.output_dir:
        ...
        csv_writer = csv.DictWriter(csv_file, fieldnames=[
            ...
            "lr_embed", "lr_head", "lr_matrix", "lr_scalar",
    ...
    while True:
        ...
        if run_val:
            ...
            log_print(f"      lr embed={opt_embed.param_groups[0]['lr']:.4g} head={opt_head.param_groups[0]['lr']:.4g} "
                      f"matrix={opt_matrix.param_groups[0]['lr']:.4g} scalar={opt_scalar.param_groups[0]['lr']:.4g}")
            log_print(f"      train_time {training_time_s:.1f}s | wall {time.time()-wall_t0:.1f}s")
        ...
        if master_process:
            ...
            lr_m = opt_matrix.param_groups[0]["lr"]
