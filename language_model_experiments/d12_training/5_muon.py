...
def get_args():
    import argparse
    p = argparse.ArgumentParser("GPT-2 Training with Muon Optimizer")

    # Data paths
    p.add_argument("--input_bin", type=str, required=True,
                   help="Glob pattern for training data shards")
    p.add_argument("--input_val_bin", type=str, default="",
...
def main():
    ...
    while True:
        ...
        if run_validation:
            ...
            # Stopping criteria - mutually exclusive modes
            epoch_frac = (train_tokens / train_loader.ntok_total) if train_loader.ntok_total > 0 else 0.0
