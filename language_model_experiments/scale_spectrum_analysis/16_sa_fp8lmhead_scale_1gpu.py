    ...
    def __init__(self, config: GPTConfig, lm_head_softcap: float = 15.0):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                h=nn.ModuleList([Block(config, i, skip_attn_layer=config.skip_attn_layer) for i in range(config.n_layer)]),
            )
        )
...
def main():
    ...
    parser.add_argument("--layers", type=str, default="auto", help="Layers to analyze")
    ...
    parser.add_argument("--num_samples_cov", type=int, default=2000)
    parser.add_argument("--cov_batch_size", type=int, default=1)
    ...
    parser.add_argument("--seq_length", type=int, default=65536, help="Sequence length for SA data loader")
    ...
    print(f"[info] model: L={cfg.n_layer} H={cfg.n_head} D={cfg.n_embd} seq_len={cfg.seq_len}")
    ...
    for path in tqdm(ckpt_paths):
        try:
            ...
            model = load_model_from_checkpoint(str(path), device, cfg=cfg, lm_head_softcap=lm_head_softcap)
            checkpoint_results = []
            ...
            for layer_idx in layers_to_analyze:
                ...
                if not cov_path.exists():
                    ...
                    dl_cov = get_data_loader(args.validation_data_path, args.num_samples_cov, args.cov_batch_size, seq_length, device)
                ...
                for matrix_name in matrices_to_analyze:
                    ...
                    if not grad_path.exists():
                        ...
                        dl_grad = get_data_loader(args.validation_data_path, args.num_samples_grad, args.grad_batch_size, seq_length, device)
