...
# -*- coding: utf-8 -*-
"""
15_sa_softcap_1gpu.py

Spectrum analysis for checkpoints produced by 15_softcap.py:
...
Only difference from 14_truncated_rope: lm_head_softcap = 15.0 (vs 30.0).
"""
...
# ==============================================================================
# 1) Model (must match 15_softcap.py)
# ==============================================================================
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig, lm_head_softcap: float = 15.0):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h=nn.ModuleList([
                Block(config.n_embd, config.n_head, i, max_seq_len=config.seq_length, skip_attn_layer=config.skip_attn_layer)
...
def embed_inputs_and_mask_sparse(
    ...
    matching 15_softcap.py's _make_block_mask.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Spectrum Analysis (15_softcap checkpoints)")
