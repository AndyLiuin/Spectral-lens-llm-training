...
#SBATCH --array=0-39
#SBATCH --job-name=sa_16_scale
#SBATCH --output=logs/sa_16_scale_%A_%a.out
#SBATCH --error=logs/sa_16_scale_%A_%a.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
          ...
          CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-data_fineweb_const_loss/16_fp8lmhead_${MODEL_PROFILE}_scale}")"
          ...
          PY_SCRIPT="${SCRIPT_DIR}/16_sa_fp8lmhead_scale_1gpu.py"
