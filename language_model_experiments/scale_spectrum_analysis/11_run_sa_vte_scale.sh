...
#SBATCH --array=0-39
#SBATCH --job-name=sa_11_scale
#SBATCH --output=logs/sa_11_scale_%A_%a.out
#SBATCH --error=logs/sa_11_scale_%A_%a.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
          ...
          CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-data_fineweb_const_loss/11_vte_${MODEL_PROFILE}_scale}")"
          ...
          PY_SCRIPT="${SCRIPT_DIR}/11_sa_vte_scale_1gpu.py"
