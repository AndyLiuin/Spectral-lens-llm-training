...
# -*- coding: utf-8 -*-
"""
13_sa_sparse_embed_1gpu.py

Spectrum analysis for checkpoints produced by 13_sparse_embed.py:
...
- args.json fallback (window schedule, dtype, model dims, block_size)
...
# ==============================================================================
# 1) Model (must match 13_sparse_embed.py)
# ==============================================================================
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, ve: Optional[torch.Tensor], block_mask: BlockMask) -> torch.Tensor:
        # x: [1, T, dim] in training; analyzer processes samples one-by-one too
        B, T, _ = x.shape
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, layer_idx: int, skip_attn_layer: int = 7):
        super().__init__()
        self.layer_idx = layer_idx
        self.skip_attn_layer = skip_attn_layer
        self.attn = CausalSelfAttention(dim, num_heads) if layer_idx != skip_attn_layer else None
        self.mlp = MLP(dim)
...
class GPTConfig:
    ...
    n_head: int = 6
    n_embd: int = 768
    seq_length: int = 65536  # analysis seq length (not training's sequence_length)
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h=nn.ModuleList([Block(config.n_embd, config.n_head, i, skip_attn_layer=config.skip_attn_layer) for i in range(config.n_layer)]),
        ))
...
def calculate_dynamic_window_tokens(step: int, window_min: int, window_max: int, window_warmup_steps: int, block_size: int) -> int:
    ...
    Match training:
      ...
      window_tokens clamped into [window_min, window_max] AFTER snapping (training effectively clamps via min/max + snapping)
    """
...
def load_model_from_checkpoint(checkpoint_path: str, device: str, cfg: GPTConfig) -> GPT:
    ...
    try:
        ...
        # (for strict=True, this should be empty; keep for sanity)
...
def main():
    parser = argparse.ArgumentParser(description="Spectrum Analysis (13_sparse_embed checkpoints)")
    ...
    # window schedule from args.json fallback
    window_min = int(train_args.get("window_min", args.window_min))
    window_max = int(train_args.get("window_max", args.window_max))
    window_warm = int(train_args.get("window_warmup_steps", args.window_warmup_steps))
    ...
    # model dims from CLI override > args.json > model profile
    ...
    print(f"[info] window schedule: min={window_min} max={window_max} warmup={window_warm} | mask block_size={block_size}")
    ...
    for path in tqdm(ckpt_paths, desc="Checkpoints"):
        ...
        # CSV handling: append logic (like scripts 3, 5, etc.)
        if results:
