...
class TrainConfig:
    ...
    batch_size: int = 64             # global batch in sequences
    device_batch_size: int = 64      # sequences per device (1-GPU)
    sequence_length: int = 1024      # tokens per sequence

    # Optimizer LRs (2-way: Embed/Head + Muon)
    embed_lr: float = 0.0036
    muon_lr: float = 0.00036
    muon_momentum: float = 0.95  # Fixed momentum
    weight_decay: float = 0.0    # Match 5_muon.py default
    ...
    wandb_run_name: str = "5_muon_sweep"
...
class GPT(nn.Module):
    ...
    def forward(self, idx, target):
        ...
        # Simple sequential pass through all blocks
        for block in self.transformer.h:
            x = block(x)
        
        x = apply_norm(x)
        logits = self.lm_head(x)
        # No softcap in 6_untie_embed
        logits = logits.float()
...
def train_one_run(
    ...
    # optimizers (use raw_model, not compiled model) - 2 optimizers for tied embed
    # All hidden layer params go to Muon
    hidden_params = list(raw_model.transformer.h.parameters())

    # Optimizer 1: AdamW for Head + Embeddings (tied)
    opt_embed = torch.optim.AdamW(
        raw_model.lm_head.parameters(),
        lr=cfg.embed_lr, betas=(0.9, 0.95),
        weight_decay=cfg.weight_decay, fused=(device_type=='cuda')
    )
    # Optimizer 2: Muon for hidden layers
    opt_muon  = Muon(hidden_params, lr=cfg.muon_lr, momentum=cfg.muon_momentum)
    optimizers = [opt_embed, opt_muon]
    ...
    with open(metrics_csv, "w", newline="") as f_csv:
        fieldnames = [
            ...
            "lr_embed","lr_muon","muon_momentum",
        ...
        while True:
            ...
            writer.writerow({
                ...
                # opt_embed covers both embed and head due to tying
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="lr_sweep/5_muon", help="Output directory")
        ...
        try:
            ...
            for bs in sorted(df["batch_size"].unique()):
                ...
                if len(bs_survivors) > 0:
                    ...
                    print(f"  Actual LRs: embed={best['embed_lr']:.4g}, "
                          f"muon={best['muon_lr']:.4g}")
