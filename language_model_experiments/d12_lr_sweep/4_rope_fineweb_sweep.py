...
class TrainConfig:
    ...
    use_cudnn_attn: bool = False  # training script uses default backends (Flash/MemEfficient)

    # WandB
    wandb_project: str = "gpt2-dynamics"
    wandb_entity: Optional[str] = None
    wandb_run_name: str = "4_rope_fineweb_sweep"
...
class GPT(nn.Module):
    ...
    def forward(self, idx, target):
        ...
        # No initial norm here? Wait, 4_rope loop matches 5_muon loop structure (norm inside block/applied before attn)
        # But 4_rope apply_norm is F.rms_norm. 
        # 5_muon.py structure: wte(idx) -> apply_norm(x) -> blocks... -> apply_norm(x) -> head
        # 4_rope.py structure: wte(idx) -> blocks... -> rms_norm(x) -> head
        # CRITICAL DIFFERENCE: 4_rope does NOT apply norm after embedding!
...
def train_one_run(
    ...
    optimizer = torch.optim.AdamW(optim_groups, lr=cfg.learning_rate, betas=(0.9, 0.95), fused=(device_type=='cuda'))
    ...
    def lr_mult(it):
        ...
        if warmdown_steps <= 0 or it < decay_start:
            return 1.0
        # Use COSINE decay to match training script (4_rope_fineweb.py)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="lr_sweep/4_rope", help="Output directory")
