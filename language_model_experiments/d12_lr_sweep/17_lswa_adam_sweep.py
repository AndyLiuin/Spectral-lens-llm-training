...
class TrainConfig:
    ...
    wandb_run_name: str = "17_lswa_adam_sweep"
...
class ASHASweepConfig:
    ...
    local_multipliers: Tuple[float, ...] = (0.5, 0.70710678, 1.0, 1.41421356, 2.0, 2.5)
...
# ============================================================
# CELL 3 — Model helpers continue below
# ============================================================
# ============================================================
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig, lm_head_softcap: float = 15.0, eot_token: int = 50256):
        super().__init__()
        self.config = config
...
def train_one_run(
    ...
    opt_matrix= torch.optim.Adam(
        matrix_params,
        lr=cfg.matrix_lr, betas=(0.9, 0.95), fused=(device_type=='cuda')
    )
    opt_scalar= torch.optim.Adam(
        scalar_params,
        lr=cfg.scalar_lr, betas=(0.8, 0.95), fused=(device_type=='cuda')
    )
    optimizers = [opt_embed, opt_head, opt_matrix, opt_scalar]
    ...
    with open(metrics_csv, "w", newline="") as f_csv:
        fieldnames = [
            ...
            "lr_embed","lr_head","lr_matrix","lr_scalar","attn_blocksize",
        ...
        while True:
            ...
            writer.writerow({
                ...
                "lr_matrix": opt_matrix.param_groups[0]["lr"],
...
def apply_lr_mult(cfg: TrainConfig, lr_mult: float) -> TrainConfig:
    ...
    c.matrix_lr = cfg.matrix_lr * lr_mult
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ASHA LR Sweep for LSWA GPT-2 Adam variant with power-law extrapolation")
    ...
    parser.add_argument("--output_dir", type=str, default="/nfs/roberts/scratch/pi_jks79/zl664/Scaling_final/const_loss_run_gpt2s/17_lswa_adam", help="Output directory")
    parser.add_argument("--batch_size", type=int, default=None,
                       help="Run the sweep for exactly one batch size. Overrides --batch_sizes when set.")
    ...
    selected_batch_sizes = [args.batch_size] if args.batch_size is not None else args.batch_sizes
    ...
    with TeeLogger(log_path):
        ...
        if args.batch_size is not None:
            print(f"Single-batch mode: batch_size={args.batch_size}")
        ...
        try:
            ...
            cols = [
                ...
                "embed_lr", "head_lr", "matrix_lr", "scalar_lr", "run_dir"
            ]
            ...
            for bs in sorted(df["batch_size"].unique()):
                ...
                if len(bs_survivors) > 0:
                    ...
                    print(f"  Actual LRs: embed={best['embed_lr']:.4g}, head={best['head_lr']:.4g}, "
                          f"matrix={best['matrix_lr']:.4g}, scalar={best['scalar_lr']:.4g}")
