...
class TrainConfig:
    ...
    # Batching (standard SDPA, no flex-attention)
    batch_size: int = 512            # global batch in sequences
    device_batch_size: int = 64      # sequences per device
    ...
    warmdown_frac: float = 1.0 # Decay to 0 over the whole run 
    # Actually 4_rope has separate warmup_iters. Sweep uses warmup_frac.
    # 4_rope default warmup_iters=0. Sweep default warmup_frac=0.0. Matches.
    # 4_rope decay depends on num_iterations. Sweep default warmdown_frac=0.1.
    # I'll keep warmdown_frac=0.1 for now, or set to 1.0 (decay all the way)?
    # 4_rope: decay_ratio from warmup to end. So effectively warmdown_frac = 1.0 (minus warmup).
    # I'll set warmdown_frac = 1.0 to match "decay to 0 from peak".
    ...
    wandb_run_name: str = "3_baseline_sweep"
...
def fit_power_law(tokens: List[float], losses: List[float], target_tokens: float) -> Tuple[float, dict]:
    ...
    # Normalize tokens to improve numerical conditioning for curve_fit.
    # Raw token counts (~1e6-1e9) cause ill-conditioned Jacobians because
    # t^(-b) produces tiny values requiring huge 'a' to compensate.
    tok_scale = tokens_arr[0]
    tokens_norm = tokens_arr / tok_scale
    target_norm = target_tokens / tok_scale

    def power_law(t, a, b, c):
        return a * np.power(t, -b) + c

    # Initial parameter guesses (using normalized tokens)
    c0 = min(losses_arr) * 0.9  # Asymptote slightly below observed minimum
    a0 = (losses_arr[0] - c0) * (tokens_norm[0] ** 0.5)  # Scale estimate
    ...
    try:
        ...
        popt, pcov = curve_fit(
            power_law, tokens_norm, losses_arr,
            p0=[a0, b0, c0],
        ...
        # Predict at target (using normalized target)
        predicted = power_law(target_norm, a, b, c)

        # Compute R² goodness of fit
        ss_res = np.sum((losses_arr - power_law(tokens_norm, *popt)) ** 2)
...
def make_cfg_for_batch(base_cfg: TrainConfig, new_batch_size: int) -> TrainConfig:
    ...
    # Note: muon_momentum is fixed, no warmup to scale
...
class GPT(nn.Module):
    ...
    def forward(self, idx: torch.Tensor, target: Optional[torch.Tensor] = None):
        B, T = idx.size()
...
def train_one_run(
    ...
    # 3_baseline.py uses simple AdamW on all params (no splitting)
    ...
    def lr_mult(it):
        ...
        if warmdown_steps <= 0 or it < decay_start:
            return 1.0
        # Use COSINE decay to match training script (3_baseline.py)
        decay_ratio = (it - decay_start) / max(1, warmdown_steps)
        decay_ratio = min(max(decay_ratio, 0.0), 1.0)
        coeff = 0.5 * (1.0 + math.cos(math.pi * decay_ratio))
        min_lr_mult = 0.1  # min_lr/max_lr ratio from training script
        return min_lr_mult + coeff * (1.0 - min_lr_mult)
    ...
    with open(metrics_csv, "w", newline="") as f_csv:
        ...
        while True:
            ...
            # Fixed muon momentum (no warmup in 6_untie_embed)
            ...
            if do_eval and val_loader is not None:
                ...
                with torch.no_grad():
                    ...
                    for _ in range(val_steps_per_eval):
                        ...
                        with ctx:
                            # Model fits in one forward pass if Batch Size is small, but if B=64 T=1024, it's 64k tokens total. 
                            # DistributedDataLoader still yields (x, y) with correct B,T.
                            # Just call model(x, y)
                            sum_val += model(xv,yv)
        ...
        if val_loader is not None:
            ...
            # Only append if this is a new data point (not a duplicate of the last one)
...
def apply_lr_mult(cfg: TrainConfig, lr_mult: float) -> TrainConfig:
    ...
    c.learning_rate = cfg.learning_rate * lr_mult
    return c
        ...
        for rung_idx, rung_tokens in enumerate(sweep.rungs):
            ...
            for i, cand in enumerate(active):
                ...
                if len(val_history) >= sweep.min_evals_for_fit:
                    ...
                    if isinstance(fit_info.get('r2'), (int, float)):
                        print(f"    Val loss: {summary['last_val']:.4f} | "
                              f"Predicted @{sweep.extrapolation_target/1e9:.0f}B: {predicted:.4f} | "
                              f"R²={fit_info['r2']:.3f}")
                    else:
                        print(f"    Val loss: {summary['last_val']:.4f} | "
                              f"Fit failed ({fit_info.get('error', 'unknown')}), fallback={predicted:.4f}")
                else:
            ...
            for cand in active[n_promote:]:
                ...
                result.update({
                    ...
                    "learning_rate": cand["cfg"].learning_rate,
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="lr_sweep/3_baseline", help="Output directory")
    parser.add_argument("--batch_sizes", type=int, nargs="+", default=[8, 16, 32, 64], help="Batch sizes to sweep")
        ...
        try:
            ...
            for bs in sorted(df["batch_size"].unique()):
                ...
                if len(bs_survivors) > 0:
                    ...
                    print(f"  Actual LR: {best['learning_rate']:.4g}")
