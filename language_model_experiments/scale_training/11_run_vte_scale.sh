...
#SBATCH --job-name=gpt2_11_scale
#SBATCH --output=logs/11_scale_%A.out
#SBATCH --error=logs/11_scale_%A.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
      ...
      OUT_DIR_DEFAULT="data_fineweb_const_loss/11_vte_${MODEL_PROFILE}_scale"
      OUT_DIR="$(resolve_path "${OUT_DIR:-${OUT_DIR_DEFAULT}}")"
      WANDB_PROJECT="${WANDB_PROJECT:-gpt2-dynamics}"
      WANDB_MODE="${WANDB_MODE:-online}"
      WANDB_RUN_NAME="${WANDB_RUN_NAME:-11_vte_scale}"
      PY_SCRIPT="${SCRIPT_DIR}/11_vte_scale.py"
