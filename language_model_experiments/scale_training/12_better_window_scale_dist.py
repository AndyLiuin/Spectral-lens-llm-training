...
from scale_arch_utils import resolve_model_dims, validate_dims
...
def get_args():
    ...
    parser.add_argument("--model_profile", type=str, default="d12", choices=["d12", "d24", "d36", "d48"],
                        help="Model profile preset")
    parser.add_argument("--n_layer", type=int, default=None, help="Number of transformer layers (override profile)")
    parser.add_argument("--n_head", type=int, default=None, help="Number of attention heads (override profile)")
    parser.add_argument("--n_embd", type=int, default=None, help="Embedding dimension (override profile)")
    parser.add_argument("--skip_attn_layer", type=int, default=-1,
                        help="Unused for variant 12; included for API compatibility")
    ...
    parser.add_argument("--loss_threshold", type=float, default=3.3, help="[const_loss mode] Early stop when val loss < threshold")
    ...
    parser.add_argument("--val_every_steps", type=int, default=100, help="Validate every N steps")
    parser.add_argument("--val_tokens", type=int, default=10485760, help="Tokens for validation")
    parser.add_argument("--checkpoint_every_steps", type=int, default=400, help="Checkpoint every N steps")
    ...
    parser.add_argument("--wandb_run_name", type=str, default="12_better_window_gpt2", help="WandB run name")
...
def main():
    ...
    log_print(f"Model profile: {args.model_profile}")
    log_print(f"Model dims: n_layer={args.n_layer}, n_head={args.n_head}, n_embd={args.n_embd}")
    ...
    opt_vte_weight = model.transformer.vte.weight
    opt_lm_head_weight = model.lm_head.weight
    opt_skip_weights = getattr(model, "skip_weights", None)
    ...
    if opt_skip_weights is not None:
        scalar_params.append(opt_skip_weights)

    optimizer1 = torch.optim.Adam(
        [opt_wte_weight, opt_vte_weight],
    ...
    while True:
        ...
        # ---- window schedule: compute CPU int so we can print it every step without CUDA sync
        warmup_frac = min(step / args.window_warmup_steps, 1.0)
        window_size = args.window_min + warmup_frac * (args.window_max - args.window_min)
        attn_blocksize_int = int(64 * (window_size // 64))
        attn_blocksize = torch.tensor(attn_blocksize_int, dtype=torch.int, device=device)
        ...
        if run_validation:
            ...
            epoch_frac = train_tokens / tokens_per_epoch if tokens_per_epoch > 0 else 0
            if args.stop_mode == "const_loss":
                # Mode 1: Stop by constant loss threshold
                if val_loss < args.loss_threshold and step > 0:
                    log_print(f"🎯 [EARLY STOP - const_loss mode] Val loss {val_loss:.4f} < threshold {args.loss_threshold:.4f}")
                    if master_process:
        ...
        if dist_env.distributed:
            all_reduce_mean_inplace(step_loss_t, dist_env)
