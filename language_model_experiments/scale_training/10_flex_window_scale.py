...
try:
    ...
    if not HELP_REQUESTED:
        raise RuntimeError("This script requires torch.nn.attention.flex_attention (flex_attention + create_block_mask).") from e
    flex_attention = None
...
os.environ["WANDB_NOTEBOOK_NAME"] = "10_flex_window_test"

def get_args():
    import argparse
    p = argparse.ArgumentParser("GPT-2 Training with Flex Window Attention")
    ...
    p.add_argument("--device_batch_size", type=int, default=1, help="Sequences per GPU (must be 1 for flex attention)")
    ...
    p.add_argument("--muon_warmup_epochs", type=float, default=None,
                   help="Epoch-based Muon warmup (overrides step-based if set)")
    ...
    p.add_argument("--window_warmup_steps", type=int, default=2000)
    ...
    p.add_argument("--wandb_run_name", type=str, default="10_flex_window_gpt2")
...
def main():
    ...
    print0(f"\n{'='*60}")
    print0("CONFIGURATION: FLEX WINDOW ATTENTION (10_flex_window)")
    print0(f"{'='*60}")
    ...
    if args.muon_warmup_epochs is not None:
        print0(f"  Muon momentum warmup: {args.muon_warmup_epochs} epochs (epoch-based)")
    else:
        print0(f"  Muon momentum warmup: {args.muon_momentum_warmup_steps} steps")

    print0(f"\nFixed Window Attention:")
    print0(f"  Window size: {args.window_min}-{args.window_max} tokens")
    ...
    print0(f"  Architecture: UNet + Flex Window Attention ({args.window_min}-{args.window_max} tokens)")
    ...
    while True:
        ...
        current_epoch_frac = epoch_idx + (step_in_epoch / max(1, epoch_steps))  # Total epoch progress (e.g., 1.5 = 1.5 epochs)
        epoch_frac = step_in_epoch / max(1, epoch_steps)  # Progress within current epoch

        # ==================== WINDOW SCHEDULE (epoch-based or step-based) ====================
        if args.window_warmup_epochs is not None:
            warmup_progress = min(current_epoch_frac / args.window_warmup_epochs, 1.0)
        else:
            warmup_progress = min(step / args.window_warmup_steps, 1.0)
        window_size = args.window_min + warmup_progress * (args.window_max - args.window_min)

        # effective window is a multiple of 64 tokens
        ...
        if run_validation:
            ...
            print0(f"      Window: {attn_blocksize_int} tokens (scheduled from {args.window_min}→{args.window_max})")
            ...
            if master_process and wandb_run is not None:
                wandb.log({
                    ...
                    "attn/window_tokens": attn_blocksize_int,
                })
            ...
            if args.stop_mode == "const_loss":
                ...
                if (args.stop_epoch_frac is not None) and (current_epoch_frac >= args.stop_epoch_frac):
                    print0(f"🎯 [EPOCH STOP] Reached {current_epoch_frac:.3f} epochs (target: {args.stop_epoch_frac})")
                    if master_process:
                        ckpt_path = save_training_checkpoint(step, raw_model, optimizers, args.output_dir, val_loss)
                        print0(f"   Checkpoint saved: {ckpt_path}")
                        saved_final_ckpt = True
        ...
        # Momentum warmup for Muon (epoch-based or step-based)
        if args.muon_warmup_epochs is not None:
            muon_frac = min(current_epoch_frac / args.muon_warmup_epochs, 1.0)
        else:
            muon_frac = min(step / args.muon_momentum_warmup_steps, 1.0)
        muon_momentum_current = (1 - muon_frac) * args.muon_momentum_init + muon_frac * args.muon_momentum_final
