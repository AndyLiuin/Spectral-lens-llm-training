...
#SBATCH --job-name=train16_dist2g
#SBATCH --output=logs/train16_dist2g_%A.out
#SBATCH --error=logs/train16_dist2g_%A.err
...
OUT_DIR="${OUT_DIR:-${SCRATCH_ROOT}/data_fineweb_const_loss_dist/16_fp8lmhead_${MODEL_PROFILE}_dist2g}"
PY_SCRIPT="${SCRIPT_DIR}/16_fp8lmhead_scale_dist.py"
...
if [[ ${status} -ne 0 && "${PARALLEL_MODE}" != "ddp" ]]; then
  ...
  echo "[WARN] FSDP run failed for variant 16, retrying with DDP at sequence_length=${FALLBACK_SEQ}"
  torchrun --standalone --nproc_per_node=2 "${PY_SCRIPT}" \
  --train_pattern "${TRAIN_PATTERN}" \
  --val_pattern "${VAL_PATTERN}" \
