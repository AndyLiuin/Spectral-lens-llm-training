...
#SBATCH --job-name=gpt_soft_spec
#SBATCH --output=logs/spec_soft_%A_%a.out
#SBATCH --error=logs/spec_soft_%A_%a.err
...
CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-data_fineweb_const_loss/15_softcap_gpt2}")"
...
PY_SCRIPT="${SCRIPT_DIR}/15_sa_softcap_1gpu.py"
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  exec sbatch \
    --chdir "${SCRIPT_DIR}" \
    --array "${ARRAY_SPEC}" \
    --output "${LOG_DIR}/spec_soft_%A_%a.out" \
    --error "${LOG_DIR}/spec_soft_%A_%a.err" \
    "${SCRIPT_PATH}"
fi
