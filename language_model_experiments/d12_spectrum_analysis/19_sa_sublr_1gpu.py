...
# =============================================================================
# Model Definitions (Matches 19_sublr.py)
# =============================================================================
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: "GPTConfig"):
        ...
        self.attn_scale = 0.12 # Added in 18/19
...
class GPT(nn.Module):
    ...
    def _make_long_short_block_masks(self, tokens_1d: torch.Tensor, sliding_window_num_blocks: int) -> tuple[BlockMask, BlockMask]:
        ...
        sw_long = torch.tensor(sliding_window_num_blocks, dtype=torch.int32, device=tokens_1d.device).clamp_min(1)
    ...
    def forward(self, idx: torch.Tensor, targets: torch.Tensor = None, attn_blocksize: int = 1024, return_logits: bool = True):
        ...
        long_bm, short_bm = self._make_long_short_block_masks(idx, sliding_window_num_blocks)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = self.transformer.h[layer_idx](x + weights_skip, ve_all[layer_idx], x0, dec_masks[i])
        ...
        if targets is not None:
            ...
            # Sigmoid softcap (from 19_sublr.py)
...
def load_model_from_checkpoint(checkpoint_path: str, device: str) -> GPT:
    ...
    if isinstance(ckpt, dict) and "args" in ckpt:
        ...
        sc = get_arg("lm_head_softcap", 30.0)
    else:
        sc = 30.0

    model = GPT(cfg, lm_head_softcap=sc) 
    state_dict = None
...
def embed_inputs_and_mask_and_vte_lswa(model: GPT, idx: torch.Tensor, attn_blocksize: int):
    idx_flat = idx.view(-1)
    BLOCK_SIZE = 128
    sliding_window_num_blocks = max(1, attn_blocksize // BLOCK_SIZE)
    
    long_bm, short_bm = model._make_long_short_block_masks(idx_flat, sliding_window_num_blocks)
    ...
    all_masks = enc_masks + dec_masks
    return x, x, ve_all, all_masks

@torch.no_grad()
def run_until_layer(model: GPT, x: torch.Tensor, x0: torch.Tensor, ve_all: list, all_masks: list, layer_idx: int) -> torch.Tensor:
    skip_connections = []
    
    for i in range(model.num_encoder_layers):
        if i > layer_idx: return apply_norm(x)
        mask = all_masks[i]
        x_out = model.transformer.h[i](x, ve_all[i], x0, mask)
        skip_connections.append(x_out)
        x = x_out

    for i in range(model.num_decoder_layers):
        current_layer_idx = model.num_encoder_layers + i
        if current_layer_idx > layer_idx: return apply_norm(x)
        
        mask = all_masks[current_layer_idx]
        skip = skip_connections.pop()
        weights_skip = model.skip_weights[i] * skip
        x = model.transformer.h[current_layer_idx](x + weights_skip, ve_all[current_layer_idx], x0, mask)
...
def compute_covariance_spectrum(model: GPT, data_loader, layer_idx: int, attn_blocksize: int) -> np.ndarray:
    ...
    for idx, _ in data_loader:
        idx = idx.to(device)
        x, x0, ve_all, all_masks = embed_inputs_and_mask_and_vte_lswa(model, idx, attn_blocksize)
        h_at = run_until_layer(model, x, x0, ve_all, all_masks, layer_idx)
...
def main():
    parser = argparse.ArgumentParser(description="GPT-2 19_sublr SA")
