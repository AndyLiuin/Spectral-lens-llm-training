...
# CUDNN Attention Configuration (MATCHING TRAINING)
try:
    ...
    print("Enabled CUDNN attention (faster than Flash)")
except Exception as e:
    print(f"Warning: Could not enable CUDNN attention: {e}")

# =============================================================================
# Model Definitions (Matches 8_unet.py)
# =============================================================================
...
class GPT(nn.Module):
    ...
    def forward(self, idx: torch.Tensor, targets: torch.Tensor = None, return_logits: bool = True):
        ...
        for i in range(self.decoder_layers):
            ...
            x, v1 = self.transformer.h[self.encoder_layers + i](x + weights_skip, v1, x0)
...
def load_model_from_checkpoint(checkpoint_path: str, device: str) -> GPT:
    ...
    # Default config based on 8_unet.py defaults
    cfg = GPTConfig(
        vocab_size=50304,
        block_size=1024,
        n_layer=12,
    ...
    # Try to infer config from usage if saved
    if isinstance(ckpt, dict) and "args" in ckpt:
        ...
        cfg = GPTConfig(
            ...
            block_size=get_arg("sequence_length", 1024),
            n_layer=get_arg("n_layer", 12),
            n_head=get_arg("n_head", 6),
            n_embd=get_arg("n_embd", 768),
        )
    else:
        # Fallback: look for args.json in the checkpoint directory
        ...
        if os.path.exists(args_path):
            ...
            cfg = GPTConfig(
             ...
             print("Warning: No args found in checkpoint or args.json. Using default GPT-2 Small config.")
...
def embed_inputs(model: GPT, idx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """Get initial embeddings and x0 state."""
...
@torch.no_grad()
def run_until_layer(model: GPT, x: torch.Tensor, x0: torch.Tensor, layer_idx: int) -> torch.Tensor:
    v1 = None
    E = model.encoder_layers
    D = model.decoder_layers
    
    cur = x
    skip_connections = []
    # Encoder
    for i in range(E):
        cur, v1 = model.transformer.h[i](cur, v1, x0)
        skip_connections.append(cur)
        if i == layer_idx:
            return F.rms_norm(cur, (cur.size(-1),), eps=RMS_EPS)

    # Decoder
    for i in range(D):
        skip_connection = skip_connections.pop()
        weighted_skip = model.skip_weights[i] * skip_connection
        cur, v1 = model.transformer.h[E + i](cur + weighted_skip, v1, x0)
        if (E + i) == layer_idx:
            return F.rms_norm(cur, (cur.size(-1),), eps=RMS_EPS)
...
def compute_covariance_spectrum(model: GPT, data_loader, layer_idx: int) -> np.ndarray:
    ...
    for idx, _ in data_loader:
        ...
        with torch.amp.autocast(device_type="cuda" if "cuda" in str(device) else "cpu", dtype=torch.bfloat16):
            x, x0 = embed_inputs(model, idx)
            h_at = run_until_layer(model, x, x0, layer_idx)
...
def main():
    parser = argparse.ArgumentParser(description="GPT-2 UNet Spectrum Analysis")
