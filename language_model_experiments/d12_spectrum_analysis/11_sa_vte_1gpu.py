...
def resolve_window_warmup_steps(checkpoint_dir: pathlib.Path, fallback_window_warmup_steps: int, reference_batch_size: int, train_args: dict):
...
def parse_layers(layers_str: str, total_layers: int) -> list[int]:
    if layers_str.lower() == "all": return list(range(total_layers))
    out = set()
...
def main():
    ...
    # Window params: window_warmup_steps is the bs8 reference schedule unless overridden.
    ...
    print(f"Window Schedule: min={wmin}, max={wmax}, warmup={wwarm} (source: {wwarm_source})")
    ...
    # Temp load for layers
    tmp = load_model_from_checkpoint(str(ckpt_paths[0]), "cpu", args.seq_length)
    ...
    for path in tqdm(ckpt_paths, desc="Checkpoints"):
        ...
        model = load_model_from_checkpoint(str(path), device, args.seq_length)
        results = []
