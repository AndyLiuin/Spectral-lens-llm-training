...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h=nn.ModuleList([Block(config.n_embd, config.n_head, i, skip_attn_layer=7) for i in range(config.n_layer)]),
        ))
...
def main():
    ...
    if args.task_index != -1:
        if args.task_index >= len(ckpt_paths):
            raise IndexError("task_index out of bounds")
    ...
    for path in tqdm(ckpt_paths, desc="Checkpoints"):
        ...
        for l in layers_to_analyze:
            ...
            for matrix_name in matrices_to_analyze:
                ...
                if not (cov_p.exists() and grad_p.exists()):
                    all_exist = False
                    break
            if not all_exist:
                break
