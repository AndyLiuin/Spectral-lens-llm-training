...
#SBATCH --job-name=gpt_fp8_spec
#SBATCH --output=logs/spec_fp8_%A_%a.out
#SBATCH --error=logs/spec_fp8_%A_%a.err
...
CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-data_fineweb_const_loss/16_fp8lmhead_gpt2}")"
...
PY_SCRIPT="${SCRIPT_DIR}/16_sa_fp8lmhead_1gpu.py"
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  exec sbatch \
    --chdir "${SCRIPT_DIR}" \
    --array "${ARRAY_SPEC}" \
    --output "${LOG_DIR}/spec_fp8_%A_%a.out" \
    --error "${LOG_DIR}/spec_fp8_%A_%a.err" \
    "${SCRIPT_PATH}"
fi
