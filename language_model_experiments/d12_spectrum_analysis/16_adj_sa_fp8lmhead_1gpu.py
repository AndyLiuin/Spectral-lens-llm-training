...
# -*- coding: utf-8 -*-
"""
16_adj_sa_fp8lmhead_1gpu.py

Spectrum analysis for checkpoints produced by 16_fp8lmhead_win_adj.py:
...
- Adjusted window schedule matching training:
    window_tokens_raw = 1728.0 * wfrac
    sw_blocks = ceil(window_tokens_raw / block_size), clamped to [1, 14]

Key differences from scripts 14/15:
  1) Window schedule: linear 0 → 1728 tokens (ref-like), NOT window_min→window_max
  2) FP8 lm_head: uses FP8 compute path matching training for gradient fidelity
  3) lm_head_softcap defaults to 15.0
"""
...
# ==============================================================================
# 1b) Model (must match 16_fp8lmhead_win_adj.py)
# ==============================================================================
...
def embed_inputs_and_mask_sparse(
    ...
    num_blocks = dense.sum(dim=-1).to(torch.int32)
    indices = torch.argsort(dense, dim=-1, descending=True, stable=True).to(torch.int32)
...
def calculate_adjusted_window_blocks(step: int, window_warmup_steps: int, block_size: int) -> Tuple[int, int]:
    """
    Match training's ADJUSTED window schedule (16_fp8lmhead_win_adj.py):
    ...
    This is DIFFERENT from the standard schedule in scripts 14/15.
...
def main():
    parser = argparse.ArgumentParser(description="Spectrum Analysis (16_fp8lmhead_win_adj checkpoints)")
    ...
    print(
        f"[info] ADJUSTED window schedule: 0→1728 tokens over {window_warm} steps "
