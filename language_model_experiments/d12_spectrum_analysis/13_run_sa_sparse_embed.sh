...
#SBATCH --job-name=gpt_sparse_spec
#SBATCH --output=logs/spec_sparse_%A_%a.out
#SBATCH --error=logs/spec_sparse_%A_%a.err
...
CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-data_fineweb_const_loss/13_sparse_embed_gpt2}")"
...
PY_SCRIPT="${SCRIPT_DIR}/13_sa_sparse_embed.py"
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  exec sbatch \
    --chdir "${SCRIPT_DIR}" \
    --array "${ARRAY_SPEC}" \
    --output "${LOG_DIR}/spec_sparse_%A_%a.out" \
    --error "${LOG_DIR}/spec_sparse_%A_%a.err" \
    "${SCRIPT_PATH}"
fi
