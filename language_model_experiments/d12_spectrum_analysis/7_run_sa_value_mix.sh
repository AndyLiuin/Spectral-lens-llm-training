...
#SBATCH --job-name=gpt_vm_spec
#SBATCH --output=logs/spec_vm_%A_%a.out
#SBATCH --error=logs/spec_vm_%A_%a.err
...
CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-/nfs/roberts/project/pi_jks79/zl664/Scaling_gpt2s/gpt_main/fineweb_const_loss/data_fineweb_const_loss/7_value_mix_gpt2}")"
...
PY_SCRIPT="${SCRIPT_DIR}/7_sa_value_mix_1gpu.py"
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  exec sbatch \
    --chdir "${SCRIPT_DIR}" \
    --array "${ARRAY_SPEC}" \
    --output "${LOG_DIR}/spec_vm_%A_%a.out" \
    --error "${LOG_DIR}/spec_vm_%A_%a.err" \
    "${SCRIPT_PATH}"
fi
