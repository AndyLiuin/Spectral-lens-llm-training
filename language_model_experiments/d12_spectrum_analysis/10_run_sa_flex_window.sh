...
#SBATCH --job-name=gpt_flex_spec
#SBATCH --output=logs/spec_flex_%A_%a.out
#SBATCH --error=logs/spec_flex_%A_%a.err
...
CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-/nfs/roberts/scratch/pi_jks79/zl664/Scaling_final/const_loss_run_gpt2s/10_flex_window_gpt2_X/bs8_elr0.6_hlr0.008_mlr0.04_slr0.04}")"
...
PY_SCRIPT="${SCRIPT_DIR}/10_sa_flex_window_1gpu.py"
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  exec sbatch \
    --chdir "${SCRIPT_DIR}" \
    --array "${ARRAY_SPEC}" \
    --output "${LOG_DIR}/spec_flex_%A_%a.out" \
    --error "${LOG_DIR}/spec_flex_%A_%a.err" \
    "${SCRIPT_PATH}"
fi
