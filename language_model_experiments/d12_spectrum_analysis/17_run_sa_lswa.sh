...
#SBATCH --job-name=gpt_lswa_spec
#SBATCH --output=logs/spec_lswa_%A_%a.out
#SBATCH --error=logs/spec_lswa_%A_%a.err
...
CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-data_fineweb_const_loss/17_lswa_gpt2}")"
...
PY_SCRIPT="${SCRIPT_DIR}/17_sa_lswa_1gpu.py"
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  exec sbatch \
    --chdir "${SCRIPT_DIR}" \
    --array "${ARRAY_SPEC}" \
    --output "${LOG_DIR}/spec_lswa_%A_%a.out" \
    --error "${LOG_DIR}/spec_lswa_%A_%a.err" \
    "${SCRIPT_PATH}"
fi
