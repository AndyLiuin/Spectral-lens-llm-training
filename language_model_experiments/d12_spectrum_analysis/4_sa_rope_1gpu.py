...
# =============================================================================
# Model Definitions (GPT-2 RoPE from 4_rope.py)
# =============================================================================
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # Enforced bias=False to match checkpoints
...
def get_data_loader(data_path: str, num_samples: int, batch_size: int, seq_length: int, device: str):
    ...
    # Needs seq_length + 1 per sample
    needed_len = seq_length + 1
    num_sequences = len(tokens) // needed_len

    if num_samples == 0 or num_samples > num_sequences:
        num_samples = num_sequences
    
    data = tokens[: num_samples * needed_len].view(num_samples, needed_len)
...
def embed_inputs(model: GPT, idx: torch.Tensor) -> torch.Tensor:
    """Get initial embeddings (token only, no positional for RoPE)."""
    return model.transformer.wte(idx)
...
def main():
    parser = argparse.ArgumentParser(description="GPT-2 RoPE Spectrum Analysis")
    ...
    ckpt_paths = sorted(
        ...
        key=lambda p: int((re.search(r"(?:state|checkpoint)(?:_target_reached)?_step(\d+)\.pt$", p.name) 
    ...
    for path in tqdm(ckpt_paths, desc="Processing checkpoints"):
        try:
            m = (re.search(r"(?:state|checkpoint)(?:_target_reached)?_step(\d+)", path.name)
                 or re.search(r"(\d+)", path.stem))
