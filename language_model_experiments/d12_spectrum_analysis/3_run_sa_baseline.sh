...
#SBATCH --job-name=gpt_baseline_spec
#SBATCH --output=logs/spec_baseline_%A_%a.out
#SBATCH --error=logs/spec_baseline_%A_%a.err
...
CHECKPOINT_DIR="$(resolve_path "${CHECKPOINT_DIR:-/nfs/roberts/project/pi_jks79/zl664/Scaling_gpt2s/gpt_main/fineweb_const_loss/data_fineweb_const_loss/3_baseline_gpt2}")"
...
PY_SCRIPT="${SCRIPT_DIR}/3_sa_baseline_1gpu.py"
TAIL_START="${TAIL_START:-30}"
TAIL_FINISH="${TAIL_FINISH:-150}"
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  exec sbatch \
    --chdir "${SCRIPT_DIR}" \
    --array "${ARRAY_SPEC}" \
    --output "${LOG_DIR}/spec_baseline_%A_%a.out" \
    --error "${LOG_DIR}/spec_baseline_%A_%a.err" \
    "${SCRIPT_PATH}"
fi
