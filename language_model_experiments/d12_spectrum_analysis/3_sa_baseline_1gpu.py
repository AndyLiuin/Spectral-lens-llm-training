...
# =============================================================================
# Model Definitions (Matches 3_baseline.py)
# =============================================================================
...
def get_data_loader(data_path: str, num_samples: int, batch_size: int, seq_length: int, device: str):
    ...
    for i in range(0, num_samples, batch_size):
        ...
        # Correct target construction: no wrap-around
        y = x.clone()
        y[:, :-1] = x[:, 1:]
        y[:, -1] = -1  # Ignore last position
        yield x, y
...
def embed_inputs(model: GPT, idx: torch.Tensor) -> torch.Tensor:
    """Get initial embeddings including positional."""
    B, T = idx.size()
    pos = torch.arange(0, T, dtype=torch.long, device=idx.device)
    return model.transformer.wte(idx) + model.transformer.wpe(pos)
...
def main():
    parser = argparse.ArgumentParser(description="GPT-2 Baseline Spectrum Analysis")
