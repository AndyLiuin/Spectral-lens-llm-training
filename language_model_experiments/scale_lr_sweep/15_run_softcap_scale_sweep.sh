...
#SBATCH --job-name=sweep_15_scale
#SBATCH --output=logs/sweep_15_scale_%A.out
#SBATCH --error=logs/sweep_15_scale_%A.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
      ...
      OUT_DIR_DEFAULT="lr_sweep/15_softcap_${MODEL_PROFILE}_scale"
      ...
      PY_SCRIPT="${SCRIPT_DIR}/15_softcap_scale_sweep.py"
