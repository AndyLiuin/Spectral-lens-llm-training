...
# ============================================================
import builtins, os, math, glob, time, json, csv
from dataclasses import dataclass, asdict
from contextlib import nullcontext
from typing import Optional, Dict, List, Tuple
from scale_arch_utils import default_batch_sizes_for_profile, resolve_model_dims, validate_dims
from scale_dist_utils import (
    all_reduce_mean_inplace,
    destroy_distributed,
    dist_barrier,
    dist_broadcast_object,
    filter_global_batch_sizes,
    init_distributed,
    promote_scalar_parameters_for_fsdp_,
...
def save_json(path: str, obj: dict) -> None:
    if DIST_ENV is not None and not DIST_ENV.master:
        return
...
class TeeLogger:
    ...
    def __enter__(self):
        ...
        self.original_stdout = builtins.print
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        builtins.print = self.original_stdout
        if self.log_file:
            self.log_file.close()
        return False
...
class TrainConfig:
    ...
    loss_fp32_enabled: bool = True

    # WandB
    wandb_project: str = "gpt2-dynamics"
    wandb_entity: Optional[str] = None
    wandb_run_name: str = "12_better_window_sweep"
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            vte = nn.Embedding(config.vocab_size, config.n_embd * (config.n_layer // 2)),
    ...
    def forward(self, idx, target, sliding_window):
        """Forward pass with improved sliding window masking (12_better_window)."""
        MASK_BLOCK_SIZE = 128
        
        # Handle both [T] and [1, T] inputs
        if idx.ndim == 2:
            assert idx.size(0) == 1, f"Expected batch=1, got {idx.shape}"
            idx = idx[0]
        if target.ndim == 2:
            assert target.size(0) == 1, f"Expected batch=1, got {target.shape}"
            target = target[0]
        ...
        def create_sliding_window_causal_mask(S: int, sliding_window: torch.Tensor):
            kv_idx = block_idx = torch.arange(S // MASK_BLOCK_SIZE, dtype=torch.int32, device="cuda")
...
def train_one_run(
    ...
    assert DIST_ENV is not None, "DIST_ENV must be initialized in main()"
    env = DIST_ENV
    assert cfg.batch_size % (cfg.device_batch_size * env.world_size) == 0
    ...
    if not env.master:
        metrics_csv = os.path.join(run_dir, f"metrics_rank{env.rank}.csv")
    ...
    raw_model.train()
    raw_model.loss_fp32 = cfg.loss_fp32_enabled

    # For explicit FSDP, normalize scalar params first so optimizer refs stay valid.
    if env.mode == "fsdp":
        promote_scalar_parameters_for_fsdp_(raw_model)

    # Extract optimizer param groups BEFORE wrapping.
    # FSDP may flatten block params after wrapping, which can make Muon's 2D split empty.
    embed_params = [raw_model.transformer.wte.weight, raw_model.transformer.vte.weight]
    ...
    if len(matrix_params) == 0:
        raise RuntimeError(
            "Muon matrix_params is empty before wrapping. "
            "Cannot initialize Muon optimizer."
        )

    model = raw_model
    if cfg.compile and device_type == "cuda" and not env.distributed:
        model = torch.compile(raw_model)
    if env.mode == "fsdp":
        model = wrap_model_fsdp(model, env, block_types=(Block,), activation_checkpointing=cfg.activation_checkpointing)
    elif env.mode == "ddp":
        model = DDP(model, device_ids=[env.local_rank])
    raw_model = model.module if env.mode in {"ddp", "fsdp"} else model

    # data
    train_loader = DistributedDataLoader(cfg.train_pattern, cfg.device_batch_size, cfg.sequence_length, env.rank, env.world_size)
    val_loader = DistributedDataLoader(cfg.val_pattern, cfg.device_batch_size, cfg.sequence_length, env.rank, env.world_size) if cfg.val_pattern else None

    grad_accum_steps = cfg.batch_size // (cfg.device_batch_size * env.world_size)
    step_tokens = cfg.batch_size * cfg.sequence_length
    print(f"[INFO] step_tokens = {step_tokens:,}")

    opt_embed = torch.optim.Adam(
        embed_params,
        lr=cfg.embed_lr, betas=(0.9,0.95), fused=(device_type=='cuda')
    )
    ...
    opt_scalar= torch.optim.Adam(
        scalar_params,
        lr=cfg.scalar_lr, betas=(0.9,0.95), fused=(device_type=='cuda')
    )
    ...
    if val_loader is not None:
        assert cfg.val_tokens % (cfg.device_batch_size * cfg.sequence_length * env.world_size) == 0
        val_steps_per_eval = cfg.val_tokens // (cfg.device_batch_size * cfg.sequence_length * env.world_size)
    ...
    with open(metrics_csv, "w", newline="") as f_csv:
        ...
        while True:
            ...
            attn_blocksize = torch.tensor(64*(window_size//64), dtype=torch.int, device=device)
            ...
            if do_eval and val_loader is not None:
                ...
                with torch.no_grad():
                    ...
                    for _ in range(val_steps_per_eval):
                        ...
                        with ctx:
                            sum_val += model(xv,yv,attn_blocksize)
                    val_avg = (sum_val / max(1, val_steps_per_eval))
                    all_reduce_mean_inplace(val_avg, env)
                    last_val_loss = float(val_avg.item())
                ...
                # (const_loss stopping is disabled when max_train_tokens is set,
                #  e.g. during ASHA sweeps that need full rung budgets)
                if cfg.stop_mode == "const_loss" and max_train_tokens is None:
            ...
            for micro_step in range(grad_accum_steps):
                # Only sync gradients on the last micro-batch (avoid redundant all-reduce)
                is_last = (micro_step == grad_accum_steps - 1)
                ctx_sync = nullcontext() if (is_last or not env.distributed) else model.no_sync()
                with ctx_sync:
                    with ctx:
                        l = model(x,y,attn_blocksize)
            ...
            train_loss_t = torch.stack(step_losses).mean()
            all_reduce_mean_inplace(train_loss_t, env)
            ...
            writer.writerow({
                ...
                "attn_blocksize": int(attn_blocksize.item()),
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                ...
                val_avg = (sum_val / max(1, val_steps_per_eval))
                all_reduce_mean_inplace(val_avg, env)
                final_val_loss = float(val_avg.item())
...
def run_asha_sweep(base_cfg: TrainConfig, sweep: ASHASweepConfig) -> List[dict]:
    ...
    for bs in sweep.batch_sizes:
        ...
        for rung_idx, rung_tokens in enumerate(sweep.rungs):
            ...
            promoted_tags = [c["tag"] for c in active[:n_promote]] if DIST_ENV.master else None
            promoted_tags = dist_broadcast_object(promoted_tags, DIST_ENV)
            active = [c for c in active if c["tag"] in set(promoted_tags)]
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="lr_sweep/12_better_window", help="Output directory")
    ...
    parser.add_argument("--parallel_mode", type=str, default="auto", choices=["auto", "single", "ddp", "fsdp"])
    parser.add_argument("--compile_mode", type=str, default="auto", choices=["auto", "on", "off"])
    parser.add_argument("--activation_checkpointing", type=str, default="auto", choices=["auto", "none", "block"])
    parser.add_argument("--loss_fp32", type=str, default="auto", choices=["auto", "on", "off"])
    ...
    global DIST_ENV
    DIST_ENV = init_distributed(args.parallel_mode, fsdp_default=False)
    ...
    compile_enabled = resolve_compile_enabled(args.compile_mode, args.model_profile, DIST_ENV.world_size)
    loss_fp32_enabled = resolve_loss_fp32_enabled(args.loss_fp32, args.model_profile)

    if args.batch_sizes is None:
        args.batch_sizes = default_batch_sizes_for_profile(args.model_profile)
    args.batch_sizes, dropped = filter_global_batch_sizes(args.batch_sizes, 1, DIST_ENV.world_size)
    if dropped and DIST_ENV.master:
        print(f"[WARN] Dropping invalid global batch_sizes for world_size={DIST_ENV.world_size}: {dropped}")
    if not args.batch_sizes:
        raise ValueError("No valid batch sizes remain after divisibility filtering")
    ...
    BASE = TrainConfig(
        ...
        loss_fp32_enabled=loss_fp32_enabled,
    )
    ...
    if not DIST_ENV.master:
        log_path = os.path.join(args.output_dir, f"asha_sweep_log_rank{DIST_ENV.rank}.txt")
    ...
    with TeeLogger(log_path):
        ...
        try:
            ...
            if not DIST_ENV.master:
                summary_csv = os.path.join(args.output_dir, f"asha_sweep_summary_rank{DIST_ENV.rank}.csv")
