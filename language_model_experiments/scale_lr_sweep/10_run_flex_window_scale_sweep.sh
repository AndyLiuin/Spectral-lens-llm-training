...
#SBATCH --job-name=sweep_10_scale
#SBATCH --output=logs/sweep_10_scale_%A.out
#SBATCH --error=logs/sweep_10_scale_%A.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
      ...
      OUT_DIR_DEFAULT="lr_sweep/10_flex_window_${MODEL_PROFILE}_scale"
      ...
      PY_SCRIPT="${SCRIPT_DIR}/10_flex_window_scale_sweep.py"
