...
#SBATCH --job-name=sweep_14_scale
#SBATCH --output=logs/sweep_14_scale_%A.out
#SBATCH --error=logs/sweep_14_scale_%A.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
      ...
      OUT_DIR_DEFAULT="lr_sweep/14_truncated_rope_${MODEL_PROFILE}_scale"
      ...
      PY_SCRIPT="${SCRIPT_DIR}/14_truncated_rope_scale_sweep.py"
