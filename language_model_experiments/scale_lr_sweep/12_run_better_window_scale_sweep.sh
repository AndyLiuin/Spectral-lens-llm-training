...
#SBATCH --job-name=sweep_12_scale
#SBATCH --output=logs/sweep_12_scale_%A.out
#SBATCH --error=logs/sweep_12_scale_%A.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
      ...
      OUT_DIR_DEFAULT="lr_sweep/12_better_window_${MODEL_PROFILE}_scale"
      ...
      PY_SCRIPT="${SCRIPT_DIR}/12_better_window_scale_sweep.py"
