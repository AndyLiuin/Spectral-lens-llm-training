...
#SBATCH --job-name=sweep_11_scale
#SBATCH --output=logs/sweep_11_scale_%A.out
#SBATCH --error=logs/sweep_11_scale_%A.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
      ...
      OUT_DIR_DEFAULT="lr_sweep/11_vte_${MODEL_PROFILE}_scale"
      ...
      PY_SCRIPT="${SCRIPT_DIR}/11_vte_scale_sweep.py"
