...
#SBATCH --job-name=sweep_13_scale
#SBATCH --output=logs/sweep_13_scale_%A.out
#SBATCH --error=logs/sweep_13_scale_%A.err
...
resolve_path() {
  case "$1" in
    ...
    *) printf '%s/%s\n' "${SA_ROOT}" "$1" ;;
      ...
      OUT_DIR_DEFAULT="lr_sweep/13_sparse_embed_${MODEL_PROFILE}_scale"
      ...
      PY_SCRIPT="${SCRIPT_DIR}/13_sparse_embed_scale_sweep.py"
